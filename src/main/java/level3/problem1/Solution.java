...
public class Solution {
  ...
  public static int bfs(int[][] map, boolean[][][] visited) {
    ...
    while (!queue.isEmpty()) {
      ...
      addNodeIfValidForNodeCreation(
          current.i + 1,
          current.j,
          current.currentPathLength,
          current.wallRemoved,
          visited,
          map,
          queue);
  ...
  public static void addNodeIfValidForNodeCreation(
      int i,
      int j,
    ...
    int visitedWithWallRemoved = wallRemoved ? 1 : 0;
    if (visited[i][j][visitedWithWallRemoved]) {
      return;
    }
    if (map[i][j] == 0) {
      visited[i][j][visitedWithWallRemoved] = true;
  ...
  public static void dfs(
      int[][] map, boolean[][] visited, boolean wallRemoved, int currentPathLength, int i, int j) {
